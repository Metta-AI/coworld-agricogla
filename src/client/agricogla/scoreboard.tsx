...
export function ScoreBoard({ state, onNewGame, onClose }: { state: GameState; onNewGame: () => void; onClose?: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 90, background: "rgba(4,6,10,0.82)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 660, maxWidth: "94vw", maxHeight: "90vh", overflowY: "auto", background: "linear-gradient(180deg, #1a2030, #11151d)", border: "1px solid #2c3548", borderRadius: 14, padding: "22px 26px", boxShadow: "0 30px 80px rgba(0,0,0,0.7)", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          <h3 style={{ margin: 0, flex: 1, fontFamily: F.display, fontWeight: 900, fontSize: 30, letterSpacing: "0.05em", textTransform: "uppercase", background: "linear-gradient(90deg, #ffffff, #ffa015)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>
            Final scoring
          </h3>
          {onClose && (
            <button onClick={onClose} aria-label="close final scoring" title="Close — explore the timeline, reopen from the FINAL SCORES chip" style={{ flex: "none", width: 30, height: 30, borderRadius: 8, background: C.field, border: `1px solid ${C.border}`, color: C.inkDim, fontSize: 15, lineHeight: 1, cursor: "pointer" }}>
              ✕
            </button>
          )}
        </div>
